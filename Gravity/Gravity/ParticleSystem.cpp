...
void ParticleSystem::add_particle(float mass, float radius, const vec2f& position, const vec2f& velocity)
{
    prt.add_particle(mass, radius, position, velocity);
    //solver.resize_buffers(prt.get_partcles().size());
    solver2.resize_buffers(prt.get_partcles().size());
}
...
void ParticleSystem::time_step(float step)
{
    ...
    solver2.integrate(prt.get_particles(), step);
}
...
void ParticleSystem::check_for_collisions()
{
    ...
    if (deleted_count > 0)
        solver2.resize_buffers(prt.get_partcles().size());//solver.resize_buffers(prt.get_partcles().size());
}
