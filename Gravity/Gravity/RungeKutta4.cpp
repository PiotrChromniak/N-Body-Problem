...
void RungeKutta4::save_initial_position(const vector<Particle> &particles) {
  std::transform(cbegin(particles), cend(particles), begin(position),
                 [&](const Particle &part) { return part.position; });
}
...
void RungeKutta4::calculate_nth_dvdt(const vector<Particle> &particles,
                                     int nth_derivative) {
  for (size_t y = 0; y < particles.size(); ++y) {
...
void RungeKutta4::get_integration_result(vector<Particle> &particles,
                                         float dt) {
  ...
  for (size_t i = 0; i < particles.size(); ++i) {
    ...
    current_particle.velocity +=
        (dVdT[0] + (dVdT[1] + dVdT[2]) * 2 + dVdT[3]) * coeff;
    const auto &dRdT = drdt[i];
    current_particle.position =
        position[i] + (dRdT[0] + (dRdT[1] + dRdT[2]) * 2 + dRdT[3]) * coeff;
  }
}
