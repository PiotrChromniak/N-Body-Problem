...
void RungeKutta4::copy_velocities_to_k_position(const vector<Particle>& particles)
{
...
void RungeKutta4::calculate_first_k_velocity(const vector<Particle>& particles) {
    for (size_t y = 0; y < particles.size(); ++y) {
        ...
        for (auto x = y + 1; x < particles.size(); ++x) {
            ...
            k_velocity[x][0] += vec2<float>{ static_cast<float>(particle2.mass * partial_acceleration * cos(angle[0])),
                static_cast<float>(partial_acceleration * particle2.mass * sin(angle[0]))};

            k_velocity[y][0] += vec2<float>{ static_cast<float>(particle1.mass * partial_acceleration * cos(angle[1])),
                static_cast<float>(partial_acceleration * particle1.mass * sin(angle[1]))};
        }
    }
}

void RungeKutta4::calculate_firt_k_position(const vector<Particle>& particles)
{
    for (size_t i = 0; i < k_velocity.size(); ++i)
        k_velocity[i][0] = particles[i].velocity;
}

void RungeKutta4::calculate_nth_k_velocity(const vector<Particle>& particles, int nth_coeff, float adjustment)
{
    for (size_t y = 0; y < particles.size(); ++y) {
        ...
        for (auto x = y + 1; x < particles.size(); ++x) {
            ...
            const auto distance = chrom::distance(adjusted_position, part1.position);
...
void RungeKutta4::calculate_nth_k_position(const vector<Particle>& particles, int nth_coeff, float adjustment)
{
    for (size_t i = 0; i < particles.size(); ++i){
...
void RungeKutta4::integrate(const vector<Particle>& particles, float time_step)
{
    reset_k_buffers();
...
void RungeKutta4::get_integration_result(vector<Particle>& particles, float time_step)
{
